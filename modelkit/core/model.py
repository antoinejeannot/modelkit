    ...
    def _validate(
        ...
        if model:
            try:
                ...
                try:
                    # pydantic 2.* does not implicitely convert
                    # a pydantic model to another one, so we need to
                    # try to convert it explicitely via a model dump
                    return model(data=item.model_dump()).data
